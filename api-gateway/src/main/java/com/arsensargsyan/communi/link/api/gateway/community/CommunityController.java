...
public class CommunityController {
    ...
    @Operation(
            summary = "${community.lookup.details.operation.summary}",
            description = "${community.lookup.details.operation.description}",
            responses = @ApiResponse(
                    content = @Content(schema = @Schema(oneOf = LookupCommunityDetailsResponse.class))
            )
    )
    @GetMapping(path = "/community/{id}/details",
            produces = MediaType.APPLICATION_JSON_VALUE
    )
    public DeferredResult<ResponseEntity<LookupCommunityDetailsResponse>> lookupDetails(
            @PathVariable("id") @NotNull @Positive Long id
    ) {
        logger.info("Lookup community details for given ID: {}.", id);
        return deferredResponse(
                CompletableFuture.supplyAsync(() -> communityClient.lookupDetails(id), executor),
                LookupCommunityDetailsResponse::new
        );
    }
    ...
    @Operation(
            summary = "${reserve.cancel.operation.summary}",
            description = "${reserve.cancel.operation.description}",
            responses = @ApiResponse(
                    content = @Content(schema = @Schema(oneOf = LookupCommunityDetailsResponse.class))
            )
    )
